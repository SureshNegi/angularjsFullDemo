...
(function ($) {
	...
	*/
    var syncCols = function (t, i, isOver) {
        ...
        //if (c.w < 40) { console.log("do not change width");return; }
        c2 = t.c[i + 1];
        c2 = t.c[t.c.length - 1];
        var w = c.w + inc;
        var w2 = c2.w - inc;
        if (w < 30)
            w = 30;
        if (w2 < 30)
            w2 = 30; //their new width is obtained					
        c.width(w + PX);
        ...
        for (var t in tables) {
            if (tables.hasOwnProperty(t)) {
                ...
                if (t.f) {                  //in fixed mode
                    ...
                    for (i = 0; i < t.ln; i++) t.c[i].css("width", ((M.round(1000 * t.c[i].w / mw) / 10) <= 7 ? 7 : (M.round(1000 * t.c[i].w / mw) / 10)) + "%").l = true;
    ...
    $.fn.extend({
        colResizable: function (options) {
            ...
            return this.each(function () {
                try { destroy($(this)); } catch (ex) { console.log() }                
                init(this, options);
            });
        }
    });
