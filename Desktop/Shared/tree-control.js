        ...
        .directive('treecontrol', ['$compile', function ($compile) {
            ...
            return {
                ...
                controller: ['$scope', '$templateCache', '$interpolate', 'treeConfig', '$rootScope', function ($scope, $templateCache, $interpolate, treeConfig, $rootScope) {
                    ...
                    $scope.expandOnOnSingleClick = function () {                                              
                        this.selectNodeHead();
                    }
                    ...
                    $scope.selectNodeOnDblClick = function (node) {
                        $scope.selectNodeLabel(node, this);
                    }
                    $scope.selectNodeLabel = function (selectedNode, dblClickedItem) {
                        $rootScope.isCompTaskDblClick = false;
                        var transcludedScope = this;
                        if (!$scope.options.isLeaf(selectedNode, $scope) && (!$scope.options.dirSelectable || !$scope.options.isSelectable(selectedNode))) {
                            ...
                            if ($scope.options.multiSelection) {
                                ...
                                if (!$scope.options.equality(selectedNode, $scope.selectedNode, $scope)) {
                                    ...
                                    selected = true;
                                    if ($scope.selectedNode.isDataNode === true && dblClickedItem) {
                                        $rootScope.isCompTaskDblClick = true;
                                    }
                                }
                                else {
                        ...
                        if (dblClickedItem && !dblClickedItem.node.isDataNode) {                           
                            dblClickedItem.selectNodeHead();
                        }
                    ...
                    if (!template) {
                        template =
                            ...
                            'set-node-to-data>' +
                            '<i class="tree-branch-head" ng-class="iBranchClass()" ng-dblClick="selectNodeHead()" ng-click="expandOnOnSingleClick()"></i>' +
                            '<i class="tree-leaf-head {{options.iLeafClass}}"></i>' +
                            '<div class="tree-label {{options.labelClass}}" ng-class="[selectedClass(), unselectableClass()]" ng-dblClick="selectNodeOnDblClick(node)" ng-click="selectNodeLabel(node)" tree-transclude></div>' +
