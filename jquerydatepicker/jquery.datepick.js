...
function getTodayButtonTooltip() {
    var calDate = new Date("sessiontimeout");
    var year = calDate.getFullYear();
    var month = (1 + calDate.getMonth()).toString();
    month = month.length > 1 ? month : '0' + month;
    var day = calDate.getDate().toString();
    day = day.length > 1 ? day : '0' + day;
    var calDateToday = (month + '/' + day + '/' + year);
    return calDateToday;
}
(function ($) { // Hide scope, no $ conflict
    ...
    $.JQPlugin.createPlugin({
        ...
        commands: {
            prev: {
                ...
                keystroke: { keyCode: 37, ctrlKey: true },//Ctrl + Left Arrow
                ...
                action: function (inst) {
                    var minDate = inst.curMinDate();
                    var previousMonth = new Date(inst.drawDate);
                    previousMonth.setMonth(previousMonth.getMonth() -1);
                    if (inst.elem[0].id == "inlineDatepicker" && minDate.getMonth() > previousMonth.getMonth() && minDate.getFullYear() >= previousMonth.getFullYear()) {
                        localStorage.isReportPopUpOpen = "true";
                        $(".DailyTimeDesktopCtrlSubmit").fadeIn();
                        setTimeout(function () {
                            $("#staticSubId").focus();
                        }, 250);
                        return;
            ...
            prevJump: {
                ...
                enabled: function (inst) {
                    var minDate = inst.curMinDate();
                    var previousYear = new Date(inst.drawDate);
                    previousYear.setFullYear(parseInt(previousYear.getFullYear()) - 1);
                    var yearNo = [];
                    yearNo[0] = parseInt(new Date("sessiontimeout").getFullYear());
                    yearNo[1] = parseInt(yearNo[0] + 1);
                    if (inst.elem[0].id == "inlineDatepicker" && previousYear.getFullYear() != yearNo[0] && previousYear.getFullYear() != yearNo[1]) {
                        localStorage.isReportPopUpOpen = "true";
                        $(".DailyTimeDesktopGridCtrl").fadeIn();
                        setTimeout(function () {
                            $("#staticId").focus();
                        }, 250);
                        return;
            ...
            next: {
                ...
                keystroke: { keyCode: 39, ctrlKey: true }, //Ctrl + Right arrow
                enabled: function (inst) {
            ...
            nextJump: {
                ...
                keystroke: { keyCode: 38, ctrlKey: true }, // Ctrl + Up Arrow
                enabled: function (inst) {
                    var maxDate = inst.get('maxDate');
                    return (!maxDate || plugin.day(plugin._applyMonthsOffset(plugin.add(
						plugin.newDate(inst.drawDate), inst.options.monthsToJump, 'm'), inst), 1).
            ...
            current: {
                ...
                keystroke: { keyCode: 32 }, // Ctrl + Home
                enabled: function (inst) {
                    var minDate = inst.curMinDate();
                    var maxDate = inst.get('maxDate');
                ...
                action: function (inst) {
                    ...
                    var target = $(this);
                    target.datepick('setDate', plugin.today()).datepick('hide');
                }
            },
            today: {
                ...
                action: function (inst) { plugin.showMonth(this); var target = $(this); target.datepick('setDate', plugin.today()).datepick('hide'); }
            },
            clear: {
	...
	alert('Now in ' + month + '/' + year);
 	...
 	alert('Selected ' + dates);
        ...
        today: function () {
            ...
            return this._normaliseDate(tempTodayDate);
        },
 ...
 $.datepick.newDate(2014, 12, 25) */
        ...
        _init: function () {
            ...
            this._super();
            this.reportStartDate = ' <div id="startDateId" class="calendarReportOver reportStartDate" style="top:-229px;">' +
                                                                '<div class="monthYearCont">' +
                                                                    '<div class="yearContainer">' +
                                                                        '<section class="navigator">' +
                                                                            '<ul>' +
                                                                                '<li><button class="leftNavigate" id="prevStartDateYear"></button></li>' +
                                                                                '<li><button class="rightNavigate" id="nextStartDateYear"></button></li>' +
                                                                            '</ul>' +

                                                                        '</section>' +
                                                                            '<div class="yearContainerinner">' +
                                                                                '<ul id="reportCalYearId"></ul>' +
                                                                        '</div>' +
                                                                        '</div>' +
                                                                    '<div class="monthContainer">' +
                                                                           ' <ul>' +
                                                                            '<li><a href="javascript:void(0)">'+ translationsEN.lbl_MnthVal1 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">'+ translationsEN.lbl_MnthVal2 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">'+ translationsEN.lbl_MnthVal3 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsEN.lbl_MnthVal4 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsEN.lbl_MnthVal5 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsEN.lbl_MnthVal6 + '</a></li>' +

                                                                            '<li><a href="javascript:void(0)">'+ translationsEN.lbl_MnthVal7 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsEN.lbl_MnthVal8 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">'+ translationsEN.lbl_MnthVal9 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsEN.lbl_MnthVal10 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsEN.lbl_MnthVal11 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsEN.lbl_MnthVal12 + '</a></li>' +
                                                                        '</ul>' +
                                                                ...
                                                                '<div class="selectOC">' +
                                                                    '<ul>' +
                                                                        '<li><button class="selectCalDate" onclick="selectStartCalDate()">OK</button></li>' +
                                                                        '<li><button class="cancelCalPopup">'+ translationsEN.btn_Cancel +'</button></li>' +
                                                                    '</ul>' +
                                                                '</div>' +
                                                           ' </div>';

            this.reportStartDateFrench = ' <div id="startDateId" class="calendarReportOver reportStartDate" style="top:-229px;">' +
                                                                '<div class="monthYearCont">' +
                                                                    '<div class="yearContainer">' +
                                                                        '<section class="navigator">' +
                                                                            '<ul>' +
                                                                    ...
                                                                    '<div class="monthContainer">' +
                                                                           ' <ul>' +
                                                                              '<li><a href="javascript:void(0)">' + translationsFR.lbl_MnthVal1 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsFR.lbl_MnthVal2 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsFR.lbl_MnthVal3 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsFR.lbl_MnthVal4 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsFR.lbl_MnthVal5 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsFR.lbl_MnthVal6 + '</a></li>' +

                                                                            '<li><a href="javascript:void(0)">' + translationsFR.lbl_MnthVal7 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsFR.lbl_MnthVal8 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsFR.lbl_MnthVal9 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsFR.lbl_MnthVal10 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsFR.lbl_MnthVal11 + '</a></li>' +
                                                                            '<li><a href="javascript:void(0)">' + translationsFR.lbl_MnthVal12 + '</a></li>' +
                                                                        '</ul>' +
                                                                ...
                                                                '<div class="selectOC">' +
                                                                    '<ul>' +
                                                                        '<li><button class="selectCalDate" onclick="selectStartCalDate()">OK</button></li>' +
                                                                        '<li><button class="cancelCalPopup">' + translationsFR.btn_Cancel + '</button></li>' +
                                                                    '</ul>' +
                                                                '</div>' +
                                                           ' </div>';

            this.reportEndDate = '<div id="endDateId"  class="calendarReportOver reportEndDate" style="top:-229px;">' +
                                                             '<div class="monthYearCont">' +
                                                                 '<div class="yearContainer">' +
                                                                         '<section class="navigator">' +
                                                                             '<ul>' +
                                                                                 '<li><button class="leftNavigate" id="prevEndDateYear"></button></li>' +
                                                                                 '<li><button class="rightNavigate" id="nextEndDateYear"></button></li>' +
                                                                             '</ul>' +
                                                             ...
                                                             '<div class="selectOC">' +
                                                                 '<ul>' +
                                                                     '<li><button class="selectCalDate" onclick="selectEndCalDate()">OK</button></li>' +
                                                                     '<li><button class="cancelCalPopup">' + translationsEN.btn_Cancel + '</button></li>' +
                                                                ' </ul>' +
                                                            ' </div>' +
                                                         '</div>';

 this.reportEndDateFrench = '<div id="endDateId"  class="calendarReportOver reportEndDate" style="top:-229px;">' +
                                                             '<div class="monthYearCont">' +
                                                                 '<div class="yearContainer">' +
                                                                         '<section class="navigator">' +
                                                                             '<ul>' +
 ...
 $(selector).datepick('setDate', [date1, date2, date3]) */
        ...
        changeMonth: function (elem, offset) {
            ...
            if (!$.isEmptyObject(inst)) {
                ...
                if (inst.elem[0].id == "startDatepicker" || inst.elem[0].id == "endDatepicker") {                    
                    var prevSevenYear = parseInt(new Date("sessiontimeout").getFullYear()) - 7;
                    var fullYear = parseInt(new Date("sessiontimeout").getFullYear()) + 1;
                    if (!(parseInt(prevSevenYear) <= parseInt(date.getFullYear()) && parseInt(date.getFullYear()) <= parseInt(fullYear))) {
                        localStorage.isReportPopUpOpen = "true";
                        $(".DailyTimeDesktopGridCtrl").fadeIn();
                        plugin.hide(elem);
                        setTimeout(function () { $("#staticId").focus(); }, 250);
                        return;
                    }
                }
                else if (inst.elem[0].id == "inlineDatepicker") {
                    var yearNo = [];
                    yearNo[0] = parseInt(new Date("sessiontimeout").getFullYear());
                    yearNo[1] = parseInt(yearNo[0] + 1);
                    if (date.getFullYear() != yearNo[0] && date.getFullYear() != yearNo[1]) {
                        localStorage.isReportPopUpOpen = "true";
                        $(".DailyTimeDesktopGridCtrl").fadeIn();
                        plugin.hide(elem);
                        setTimeout(function () { $("#staticId").focus();}, 250);
        ...
        _generateContent: function (elem, inst) {
            inst.options.renderer.picker = "";
            var pickerHtml = '<div class="datepick ' + inst.elem[0].id + 'cls"><div class="datepick-nav">{link:prev}{link:today}{link:next}</div>{months}{popup:start}<div class="datepick-ctrl">{link:clear}{link:close}</div>{popup:end}<div class="datepick-clear-fix"></div></div>';
            inst.options.renderer.picker = pickerHtml;
            if (inst.elem[0].id == "startDatepicker" && inst.options.renderer.picker.indexOf('startDateId') < 0) {
                if (localStorage.isEnglishLang == "true")
                    inst.options.renderer.picker = this.reportStartDate + inst.options.renderer.picker;
                else
                    inst.options.renderer.picker = this.reportStartDateFrench + inst.options.renderer.picker;
            }
            else if (inst.elem[0].id == "endDatepicker" && inst.options.renderer.picker.indexOf('endDateId') < 0) {
                if (localStorage.isEnglishLang == "true")
                    inst.options.renderer.picker = this.reportEndDate + inst.options.renderer.picker;
                else
                    inst.options.renderer.picker = this.reportEndDateFrench + inst.options.renderer.picker;
            }
            var monthsToShow = inst.options.monthsToShow; reportStartDate = ""; reportEndDate = "", reportStartDateFrench = "", reportEndDateFrench="";
            ...
            var addCommand = function (type, open, close, name, classes) {
                ...
                picker = picker.replace(new RegExp('\\{' + type + ':' + name + '\\}', 'g'),
					'<' + open + (command.status ? ' title="' + (inst.options[command.status].indexOf("(Spacebar)") > -1 ? getTodayButtonTooltip() + ' (Spacebar)' : inst.options[command.status]) + '"' : '') +
        ...
        _generateMonthSelection: function (inst, year, month, minDate, maxDate, monthHeader, renderer) {
            ...
            var selectedMonthName = "", selectedYear = 0;
            for (var m = 1; m <= 12; m++) {
                if ((!minDate || plugin.newDate(year, m, plugin.daysInMonth(year, m)).
                    ...
                    if (month === m) {
                        selectedMonthName = monthNames[m - 1];
                        selectedYear = year;
                    }
                }
            }
            selector += '</select>'; var selector = '';
            if (inst.elem[0].id == "inlineDatepicker")
                selector = '<span id="submitDtId"><strong onclick="showSubmitRevampCalendar()" class="ng-binding">' + selectedMonthName + ' ' + selectedYear + '   <i class="fa fa-caret-down" aria-hidden="true"></i></strong></span>'
            else if (inst.elem[0].id == "startDatepicker")
                selector = '<span id="startDtId"><strong  onclick="showStartDateRevampCalendar()" class="ng-binding">' + selectedMonthName + ' ' + selectedYear + '   <i class="fa fa-caret-down" aria-hidden="true"></i></strong></span>'
            else if (inst.elem[0].id == "endDatepicker")
                selector = '<span id="endDtId"><strong onclick="showEndDateRevampCalendar()" class="ng-binding">' + selectedMonthName + ' ' + selectedYear + '   <i class="fa fa-caret-down" aria-hidden="true"></i></strong></span>'
